...
########## in case under some circumstance, will get URL certificate failure. ################
...
class ClassBlock(nn.Module):
    def __init__(self, input_dim, class_num, droprate, relu=False, bnorm=True, num_bottleneck=512, linear=True, return_f = False, return_mid = False):
...
class plain_lenet(nn.Module):

    def __init__(self, class_num, droprate=0.5, stride=2):
        super(plain_lenet, self).__init__()
...
class resnet50_middle(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, pretrain= True, return_f = False, return_mid = False):
        super(resnet50_middle, self).__init__()
        model_ft = models.resnet50(pretrained=pretrain)
        # avg pooling to global pooling
        model_ft.avgpool = nn.AdaptiveAvgPool2d((1,1))
        self.model = model_ft
        self.classifier = ClassBlock(2048+1024, class_num, droprate, return_f= return_f, return_mid=return_mid)
...
# Test the model, before you train it. 
    net = resnet50_middle(352, droprate=0.5,pretrain=False)
